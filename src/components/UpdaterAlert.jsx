...
export default function UpdaterAlert({ open, force, onUpdate, onClose, releaseNotes, theme }) {
  if (!open) return null;

  const t = theme; // Expecting theme object

  // The alert behaves differently based on `force`
  if (force) {
    return (
      ...
      <div className={`pointer-events-auto ${t.bgCard} ${t.textMain} rounded-2xl p-4 shadow-2xl ${t.shadowAccent} w-full max-w-sm border ${t.borderAccentSoft} flex flex-col gap-3 relative overflow-hidden backdrop-blur-xl`}>
        ...
        <div className="flex gap-4 items-center pr-6">
          ...
          <div>
            ...
            <p className={`text-xs ${t.textMuted} mt-0.5`}>Ada versi Lomeal yang lebih baru.</p>
          </div>
        </div>
